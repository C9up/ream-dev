/**
 * Ream Demo — Ignitor + HyperServer NAPI + events + Spectrum + Rune.
 *
...
import { EventBus } from '@c9up/ream/events'
...
<body>
  <div class="c">
    ...
    <p class="v">Ream v0.1.0 — Hyper + events + Spectrum + Rune</p>
  </div>
</body>
</html>`)
  })
  ...
  router.get('/api/health', async ({ response }) => {
    response.status(200).json({
      ...
      stack: ['hyper', 'events', 'spectrum', 'rune'],
    })
  })
  ...
  router.get('/api/bus-test', async ({ response }) => {
    ...
    const event = JSON.parse(eventJson)
    response.status(200).json({
      message: 'Event emitted on the bus',
...
logger.info('Stack: Hyper (Rust) + events (Rust) + Spectrum + Rune')
