...
| Package | Description |
|---|---|
| `@c9up/ream` | Core — Ignitor, IoC container, router, middleware, lifecycle, session, mail, hash, security |
| `@c9up/pulsar` | Event bus — emit, subscribe, wildcards, request-reply, correlation IDs (Rust NAPI) |
...
7. `cargo check` on workspace-EXCLUDED crates
   (`packages/pulsar/crates/pulsar-bus-napi` + `packages/ream-cli`,
   neither covered by `cargo check --all`)
